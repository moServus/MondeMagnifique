---
layout: default
title: Mo Servus - Progetti
lang: it
---

[🇦🇹 Deutsch](./index-de.html) | [🇫🇷 Français](./index-fr.html) | [🇮🇹 Italiano](./index-it.html) | [🇬🇧 English](./index.html)

Benvenuti nel mio portafoglio di progetti open-source e guide di viaggio.

**Repository GitHub**: [moservus/MondeMagnifique](https://github.com/moServus/MondeMagnifique)

---

## 🎯 Viaggi in Evidenza

### 🇦🇹 Mein Wien - Guida di Viaggio a Vienna
**La Tua Guida Completa a Vienna, Austria**

Una guida di viaggio completa e facile da usare per Vienna con fonti Wikipedia selezionate, approfondimenti storici e informazioni pratiche di viaggio.

- 📍 **Posizione**: Nordest dell'Austria sul Danubio
- 🏛️ **Storia**: Da Vindobona romana attraverso l'Impero austro-ungarico ai tempi moderni
- 🎫 **Da Vedere**: Palazzo Schönbrunn, Cattedrale di Santo Stefano, Hofburg
- 🎵 **Cultura**: 100+ musei, cultura del caffè UNESCO, eredità della musica classica
- 🍷 **Gastronomia**: Schnitzel viennese, Sachertorte, autentiche taverne Heuriger

**Stato**: 🏋🏼‍♂️ Lavori in Corso | Ricerca iniziata il 20 giugno 2026 | Viaggio: 15-17 agosto 2026

[**Esplora la Guida di Vienna →**](./wien-it.html)

---

## 🔗 Link Rapidi

- **Repository GitHub**: [moservus/MondeMagnifique](https://github.com/moServus/MondeMagnifique)
- **Guida di Vienna**: [Guida Completa di Viaggio](./wien-it.html)
- **Riferimenti**: [Riferimenti Wikipedia](./wikiViennaDE.md)

---

<div style="text-align: center; margin-top: 3rem; padding-top: 2rem; border-top: 1px solid #eee;">
  <p><em>Esplorare il mondo una destinazione alla volta.</em></p>
  <p>Altri progetti in arrivo presto...</p>
  <p>Fatto con ❤️ per i viaggiatori e gli appassionati di open-source</p>
</div>
